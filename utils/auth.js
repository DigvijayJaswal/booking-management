...
auth.verifyApiKey = async (req, res, next) => {
	if (req.query && req.query.apiKey && req.query.apiKey === (process.env.API_KEY || 'booking!@#$%^&*&^&**^$')) {
		next();
	} else if (req.body && req.body.apiKey && req.body.apiKey === (process.env.API_KEY || 'booking!@#$%^&*&^&**^$')) {
		next();
	} else {
		res.status(401).json({
			success: 'failure',
